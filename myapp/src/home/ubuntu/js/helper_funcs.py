...
def load_data(subfolder,opt,generator=False):
    def create_image_array(image_list, image_path, image_size, nr_of_channels):
        ...
        widgets = [
            ...
            ' ',
            progressbar.Bar(),  # 显示进度条
            ' ',
            progressbar.ETA()  # 显示预计剩余时间（可选）
        ]
        bar = progressbar.ProgressBar(
            widgets=widgets,
            max_value=len(image_list),
            prefix='Loading images'  # 自定义前缀
        )
...
def load_negative_data(image_folder,opt):
    ...
    if image_folder == 'T1-T2':
        ...
        shuffle(file_CT)
        count = 0
        image_array_ct = np.empty((opt['batch_size'],) + ((512,512)) + (1,))
        image_array_mri = np.empty((opt['batch_size'],) + ((512,512)) + (1,))
        for count in range(opt['batch_size']):
            ...
            trip_image_MRI = resize(trip_image_MRI,(512,512))
            ...
            trip_image_CT = resize(trip_image_CT,(512,512))
...
def save_model(opt, model, epoch):
    ...
    directory = os.path.join('/home/ubuntu/js/', 'saved_models', opt['date_time'])
